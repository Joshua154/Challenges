...
public class temp {
    private void temp() {
        Map<Material, Color> blocksMap = new HashMap<Material, Color>();
    ...
    private void test(Path path, UserPrincipal user, GroupPrincipal group) throws IOException {
        Set<PosixFilePermission> ownerWritable = PosixFilePermissions.fromString("rw-rw-r--");

        var file = Files.getFileAttributeView(path, PosixFileAttributeView.class, LinkOption.NOFOLLOW_LINKS);

        file.setPermissions(ownerWritable);
        file.setOwner(user);
        file.setGroup(group);
    }
}
